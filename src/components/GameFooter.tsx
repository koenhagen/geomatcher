...
export const GameFooter: React.FC<GameFooterProps> = ({ totalScore, slotsFilled, totalSlots, round, maxRounds, onSubmit }) => {
    const isComplete = slotsFilled === totalSlots;
    const progressPercent = (slotsFilled / totalSlots) * 100;
    const isFinalRound = round >= maxRounds;

    return (
        <div className="fixed bottom-0 left-0 w-full bg-background-dark/90 backdrop-blur-xl border-t border-slate-800 p-5 pb-8 space-y-5 z-100 shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
            ...
            <div className="max-w-7xl mx-auto px-1">
                <button
                    disabled={!isComplete}
                    onClick={onSubmit}
                    className={`flex w-full items-center justify-center overflow-hidden rounded-2xl h-16 px-4 text-base font-black uppercase tracking-widest transition-all
            ${isComplete
